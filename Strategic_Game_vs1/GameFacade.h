...
public:
	...
	void addUnit(ClientInput* input) {
		game->addUnit(input->getRow(), input->getCol(),input->getUnitType(),input->getColor(),curPlayer);
		switchPlayer();
	}

	void moveUnit(ClientInput* input){
		game->moveUnit(input->getRow(), input->getCol(), input->getRowTo(), input->getColTo(), curPlayer);
		switchPlayer();
	}
